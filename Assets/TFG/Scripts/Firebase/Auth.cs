...
// startup.
namespace TFG.Authentification
{
    public class Auth : Singleton<Auth>
    {
        ...
        {
            Debug.Log("<color=#" + ColorUtility.ToHtmlStringRGB(Color.cyan) + ">" + s + "</color>");
            logText += s + "\n";
        ...
        protected bool LogTaskCompletion(Task task, string operation)
        {
            ...
            else if (task.IsFaulted)
            {
                ...
                foreach (Exception exception in task.Exception.Flatten().InnerExceptions)
                {
                    ...
                    loginError = authErrorCode + exception.ToString();
                    DebugLog(loginError);
                }
            }
        ...
        {
            DebugLog(String.Format("Attempting to create user {0}...", _user.text)); //EmailMOD

            // This passes the current displayName through to HandleCreateUserAsync
            // so that it can be passed to UpdateUserProfile().  displayName will be
            // reset by AuthStateChanged() when the new user is created and signed in.
            string newDisplayName = displayName;
            return auth.CreateUserWithEmailAndPasswordAsync(_user.text, _password.text) //EmailMOD PassMOD
        ...
        {
            DebugLog(String.Format("Attempting to sign in as {0}...", _user.text)); //EmailMOD
            if (signInAndFetchProfile)
            {
                return auth.SignInAndRetrieveDataWithCredentialAsync(
                  Firebase.Auth.EmailAuthProvider.GetCredential(_user.text, _password.text)).ContinueWithOnMainThread( //EmailMOD PassMOD
                    HandleSignInWithSignInResult);
            }
            else
            {
                return auth.SignInWithEmailAndPasswordAsync(_user.text, _password.text) //EmailMOD PassMOD
        ...
        public Task SigninWithEmailCredentialAsync()
        {
            ...
            {
                return auth.SignInAndRetrieveDataWithCredentialAsync(
                  Firebase.Auth.EmailAuthProvider.GetCredential(_user.text, _password.text)).ContinueWithOnMainThread(//EmailMOD
                    HandleSignInWithSignInResult);
            }
            else
            {
        ...
        protected Task LinkWithEmailCredentialAsync()
        {
            ...
            Firebase.Auth.Credential cred =
              Firebase.Auth.EmailAuthProvider.GetCredential(_user.text, _password.text); //EmailMOD PassMOD
            if (signInAndFetchProfile)
            {
                return
        ...
        protected Task ReauthenticateAsync()
        {
            ...
            Firebase.Auth.Credential cred = Firebase.Auth.EmailAuthProvider.GetCredential(_user.text, _password.text);//EmailMOD PassMOD
            if (signInAndFetchProfile)
            {
        ...
        protected Task UnlinkEmailAsync()
        {
            ...
            return auth.CurrentUser.UnlinkAsync(
              Firebase.Auth.EmailAuthProvider.GetCredential(_user.text, _password.text).Provider) //EmailMOD PassMOD
                .ContinueWithOnMainThread(task =>
                {
        ...
        public void SignOut()
        {
            ...
            auth.SignOut();
            if (UserEvents.userSignOut != null)
            {
                UserEvents.userSignOut.Invoke();
            }
        }
        ...
        {
            auth.FetchProvidersForEmailAsync(_user.text).ContinueWithOnMainThread((authTask) =>
            { //EmailMOD
                if (LogTaskCompletion(authTask, "Fetch Providers"))
                {
                    DebugLog(String.Format("Email Providers for '{0}':", _user.text)); //EmailMOD
        ...
        {
            auth.SendPasswordResetEmailAsync(_user.text).ContinueWithOnMainThread((authTask) =>
            { //EmailMOD
                if (LogTaskCompletion(authTask, "Send Password Reset Email"))
                {
                    DebugLog("Password reset email sent to " + _user.text); //EmailMOD
                }
            });
        }
        ...
        #region TFG Firebase
        public void TFGSignInEmail(TMP_InputField user, TMP_InputField pass)
        {
            _user = user;
            _password = pass;
